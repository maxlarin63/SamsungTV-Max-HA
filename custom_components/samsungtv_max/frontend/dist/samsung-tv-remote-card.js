...
`;const ut="samsung-tv-remote-card",_t=window;_t.customCards=_t.customCards||[],_t.customCards.some(t=>t.type===ut)||_t.customCards.push({type:ut,name:"Samsung TV Remote",description:"Full remote control for Samsung Tizen TVs (Samsung TV Max)"});const mt=[{key:"_POWER",icon:"mdi:power",label:"ON"},{key:"KEY_MENU",icon:"mdi:menu",label:"Menu"}],$t=[{key:"KEY_VOLUP",icon:"mdi:volume-plus",label:"Vol +",hold:!0},{key:"KEY_MUTE",icon:"mdi:volume-off",label:"Mute"},{key:"KEY_CHUP",icon:"mdi:arrow-up-bold",label:"CH up",hold:!0}],ft=[{key:"KEY_VOLDOWN",icon:"mdi:volume-minus",label:"Vol −",hold:!0},{key:"KEY_SOURCE",icon:"mdi:video-input-hdmi",label:"Source"},{key:"KEY_CHDOWN",icon:"mdi:arrow-down-bold",label:"CH dn",hold:!0}],yt=[{key:"KEY_HOME",icon:"mdi:home",label:"Home"},{key:"KEY_UP",icon:"mdi:arrow-up",hold:!0},{key:"KEY_INFO",icon:"mdi:information",label:"Info"}],gt=[{key:"KEY_LEFT",icon:"mdi:arrow-left",hold:!0},{key:"KEY_ENTER",icon:"mdi:keyboard-return",label:"OK"},{key:"KEY_RIGHT",icon:"mdi:arrow-right",hold:!0}],vt=[{key:"KEY_RETURN",icon:"mdi:keyboard-backspace",label:"Back"},{key:"KEY_DOWN",icon:"mdi:arrow-down",hold:!0},{key:"KEY_EXIT",icon:"mdi:close",label:"Exit"}],bt=[{service:"media_play",icon:"mdi:play",label:"Play"},{service:"media_pause",icon:"mdi:pause",label:"Pause"},{service:"media_stop",icon:"mdi:stop",label:"Stop"},{service:"media_previous_track",icon:"mdi:skip-previous",label:"Prev"},{service:"media_next_track",icon:"mdi:skip-next",label:"Next"}],At=[{name:"YouTube",icon:"mdi:youtube",label:"YT"},{name:"Netflix",icon:"mdi:netflix",label:"Netflix"},{name:"Spotify",icon:"mdi:spotify",label:"Spotify"},{name:"Browser",icon:"mdi:earth",label:"Web"}];class Et extends ot{constructor(){super(...arguments),this.connectedWhileHidden=!0,this._textValue="",this._holdsBound=!1}setConfig(t){this._config=t?{...t}:void 0}getCardSize(){return 8}updated(t){super.updated(t),!this._holdsBound&&this.hass&&this._config&&this._bindHoldButtons()}render(){try{return this._renderCard()}catch(t){return D`<ha-card><div class="status">Render error: ${String(t)}</div></ha-card>`}}_renderCard(){if(!this._config)return D``;if(!this._config.entity)return D`<ha-card><div class="status">Set "entity" to a remote.* entity in card config</div></ha-card>`;if(!this.hass)return D`<ha-card><div class="status">Loading…</div></ha-card>`;const t=this.hass.states[this._config.entity];if(!t)return D`<ha-card><div class="status">Entity not found: ${this._config.entity}</div></ha-card>`;const e=t.attributes,i="on"===t.state;return D`
      <ha-card>
        ${this._renderRow(mt,"row-2",i)}
        ${e.keyboard_active?this._renderTextInput():B}
        ${this._renderRow($t,"row-3")}
        ${this._renderRow(ft,"row-3")}
        ${this._renderRow(yt,"row-3")}
        ${this._renderRow(gt,"row-3")}
        ${this._renderRow(vt,"row-3")}
        ${this._renderTransport(e)}
        ${this._renderApps(e)}
        <div class="status">
          ${e.tv_model||"Samsung TV"} &middot; ${e.power_state??"unknown"}
        </div>
      </ha-card>
